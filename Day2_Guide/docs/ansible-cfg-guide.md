...
sudo cp /tmp/repo/Day2_Guide/inventory/my_inventory.sh /opt/ansible-env/inventory/my_inventory.sh
...
저장소의 `Day2_Guide/inventory/my_inventory.sh` 가 "원본", Agent 의 `/opt/ansible-env/inventory/my_inventory.sh` 가 "실제 실행되는 파일". 스크립트를 수정하면 각 Agent 에 다시 복사해야 한다.
